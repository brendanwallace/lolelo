...
def update_ratings_and_predictions():
    ...
    teams = {
        team.name: team
        for team in na_lcs_models.Team.objects.all()
    }
    spring_matches = na_lcs_models.Match.objects.filter(season__name="Spring 2017").order_by('game_number')
    summer_matches = na_lcs_models.Match.objects.filter(season__name="Summer 2017").order_by('game_number')
    ...
    for match in spring_matches:
        if match.team_1_wins + match.team_2_wins > 0:
            team_1_expected_sna_lcs = (
                expected_outcome(teams[match.team_1.name].rating, teams[match.team_2.name].rating) *
                (match.team_1_wins + match.team_2_wins)
            )
            # should be the same as (res1 + res2) - team1_expected_sna_lcs
            team_2_expected_sna_lcs = (
                expected_outcome(teams[match.team_2.name].rating, teams[match.team_1.name].rating) *
                (match.team_1_wins + match.team_2_wins)
            )
            teams[match.team_1.name].rating += K_FACTOR * (match.team_1_wins - team_1_expected_sna_lcs)
            teams[match.team_2.name].rating += K_FACTOR * (match.team_2_wins - team_2_expected_sna_lcs)
